...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "grupo15-ear.ear");

        // Current directory is the root of the "grupo15-web" project. Go up one level
        // and enter the "grupo15-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../grupo15-ejb/target/grupo15-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "grupo15-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/grupo15-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "grupo15-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("grupo15.grupo15.test");
