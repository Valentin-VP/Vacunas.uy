...
public class GestionEtapaRWS {
	...
	@GET
	@Path("/obtener")
	public Response obtenerEtapa(@CookieParam("x-access-token") Cookie cookie, @QueryParam("p") String plan,
			@QueryParam("e") String etapa) {
	...
	public Response agregarEtapa(String datos) {
		try {
			...
			ce.agregarEtapa(LocalDate.parse(etapa.getString("fechaInicio"), formatter),
					LocalDate.parse(etapa.getString("fechaFin"), formatter), etapa.getString("condicion"),
					Integer.parseInt(etapa.getString("plan")), etapa.getString("vacuna"));
			return ResponseBuilder.createResponse(Response.Status.OK, "Se ha agregado la etapa con exito");
		} catch (NumberFormatException | EtapaRepetida | PlanVacunacionInexistente | VacunaInexistente | AccionInvalida
				| JSONException e) {
			//
			return ResponseBuilder.createResponse(Response.Status.BAD_REQUEST, e.getMessage());
		}
	...
	@DELETE
	@Path("/eliminar")
	public Response eliminarEtapa(@CookieParam("x-access-token") Cookie cookie, @QueryParam("e") String etapa,
			@QueryParam("p") String plan) {
	...
	public Response modificarEtapa(String datos) {
		...
		try {
			...
			LocalDate fechaInicio = LocalDate.of(Integer.valueOf(fi[0]), Integer.valueOf(fi[1]),
					Integer.valueOf(fi[2]));
