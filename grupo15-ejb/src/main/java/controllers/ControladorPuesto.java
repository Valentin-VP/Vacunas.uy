...
public void agregarPuesto(String id, Vacunatorio vacunatorio) throws PuestoCargadoException {
	...
	if (puesto==null) {
		Puesto puestoNew= new Puesto(id, vacunatorio);
...
public DtPuesto obtenerPuesto(String id) throws PuestoNoCargadoException {
	...
	throw new PuestoNoCargadoException("El puesto "+ id +" no existe en el sistema\n");
		...
		DtPuesto dtPuesto = new DtPuesto(puesto.getId(), puesto.getVacunatorio());
				return dtPuesto;
	}
...
public ArrayList<DtPuesto> listarPuestos()throws PuestoNoCargadosException {
	...
	for(Puesto p: aux ) {
		...
		DtPuesto dtPuesto = new DtPuesto(p.getId(), p.getVacunatorio());
...
public void modificarPuesto(DtPuesto dtPuesto) throws PuestoNoCargadoException {
	...
	puesto.setVacunatorio(dtPuesto.getVacunatorio());
