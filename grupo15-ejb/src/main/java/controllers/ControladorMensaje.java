...
public void agregarMensaje(String contenido)  {
	...
	public DtMensaje BuscarMensaje(int IdMensaje) throws MensajeExistente {
		...
		if (em.find(Mensaje.class,IdMensaje) == null) {
			throw new MensajeExistente("No existe el mensaje ingresado");
		}
		
		Mensaje c = em.find(Mensaje.class,IdMensaje);
		DtMensaje dt = new DtMensaje(c.getIdMensaje(), c.getContenido());
		return dt;
	}
	...
	public void ModificarMensaje(DtMensaje mensaje) throws MensajeExistente {
		...
		if (em.find(Mensaje.class,mensaje) == null) {
			throw new MensajeExistente("No existe el mensaje ingresado");
		}
		...
		c.setContenido(mensaje.getContenido());
		em.persist(c);
	...
	public ArrayList<DtMensaje> listarMensajes() {
		...
		Query q = em.createQuery("select u from Mensaje u");
		ArrayList<Mensaje> mensajes = (ArrayList<Mensaje>) q.getResultList();
		
		for(Mensaje u: mensajes) {
			chat.add(new DtMensaje(u.getIdMensaje(), u.getContenido()));
		}
